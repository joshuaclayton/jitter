...
task :build do
  Dir.chdir(JITTER_SOURCE_DIR) do
    File.open(output_path = File.join(JITTER_BUILD_DIR, 'jquery.jitter.js'), 'w+') do |result|
      ...
      ["jquery.jitter.core", "jquery.jitter.defaults", "jquery.jitter.errors", "jquery.jitter.feeds", "jquery.jitter.builder", "jquery.jitter.utilities", "jquery.fn.jitter"].each do |js_file|
